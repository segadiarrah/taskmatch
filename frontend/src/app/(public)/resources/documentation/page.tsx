      ...
      <section className="relative overflow-hidden border-b border-ink-800 bg-ink-950 px-4 pb-16 pt-28 sm:px-6 lg:px-8">
        ...
        <div className="relative mx-auto max-w-7xl">
          <div className="max-w-3xl">
            <Reveal className="inline-flex items-center gap-2 rounded-full border border-ink-700 bg-ink-900/80 px-4 py-1.5 eyebrow text-ink-300">
              <BookOpen className="h-3.5 w-3.5 text-signal-400" />
              {c.eyebrow}
            </Reveal>
            <Reveal delay={80}>
              <h1 className="mt-8 font-display text-5xl font-medium leading-[1.02] text-ink-50 sm:text-6xl">
                {c.title}
                <span className="block font-display italic text-signal-500">{c.accent}</span>
              </h1>
            </Reveal>
            <Reveal delay={160}>
              <p className="mt-6 max-w-2xl text-lg leading-8 text-ink-300">{c.description}</p>
            </Reveal>
          </div>
        </div>
      </section>
      ...
      <div className="mx-auto flex max-w-7xl gap-8 px-4 py-10 sm:px-6 lg:px-8">
        <aside className="hidden w-64 shrink-0 lg:block">
          <div className="sticky top-24 rounded-lg border border-ink-700 bg-ink-900 p-4">
            <div className="mb-3 px-2 eyebrow text-ink-500">{c.onThisPage}</div>
            <nav className="space-y-1">
              {sectionMeta.map((section) => (
                <a
                  key={section.id}
                  href={`#${section.id}`}
                  className="flex items-center gap-3 rounded-md px-3 py-3 text-sm font-medium text-ink-300 transition-colors hover:bg-ink-800 hover:text-ink-50"
                >
        ...
        <button
          className="fixed bottom-6 right-6 z-40 inline-flex h-12 w-12 items-center justify-center rounded-full bg-signal-500 text-ink-950 shadow-glow-sm lg:hidden"
        ...
        {mobileOpen ? (
          <div className="fixed inset-0 z-30 bg-black/60 lg:hidden" onClick={() => setMobileOpen(false)}>
            <div
              className="absolute bottom-20 right-6 w-64 rounded-lg border border-ink-700 bg-ink-900 p-3 shadow-panel"
            ...
            >
              {sectionMeta.map((section) => (
                <a
                  ...
                  className="flex items-center gap-3 rounded-md px-3 py-3 text-sm font-medium text-ink-300 hover:bg-ink-800 hover:text-ink-50"
                >
        ...
        <main className="min-w-0 flex-1 space-y-12">
          <Reveal as="section" className="rounded-lg border border-ink-700 bg-ink-900 p-8" >
            <span id="getting-started" className="-mt-24 block pt-24" aria-hidden />
            <h2 className="font-display text-3xl font-medium text-ink-50">{c.labels["getting-started"]}</h2>
            ...
            <div className="mt-8 grid gap-4 md:grid-cols-3">
              {[
                ...
                <div key={item} className="rounded-md border border-ink-700 bg-ink-850 p-5">
                  <div className="eyebrow text-signal-400">0{index + 1}</div>
                  <p className="mt-4 text-sm leading-7 text-ink-300">{item}</p>
                </div>
              ))}
            </div>
            <div className="mt-8">
            ...
            <div className="mt-8 grid gap-4 md:grid-cols-3">
              {[
                ...
                <div key={item.title} className="rounded-md border border-ink-700 bg-ink-850 p-5">
          ...
          <Reveal as="section" className="rounded-lg border border-ink-700 bg-ink-850 p-8">
            <span id="architecture" className="-mt-24 block pt-24" aria-hidden />
            <h2 className="font-display text-3xl font-medium text-ink-50">{c.labels.architecture}</h2>
            <p className="mt-4 max-w-3xl text-base leading-8 text-ink-300">
            ...
            <div className="mt-8 grid gap-5 md:grid-cols-4">
              {["Client request", "Task structuring", "Agent execution", "Validation and delivery"].map((item) => (
                <div key={item} className="rounded-md border border-ink-700 bg-ink-900 p-5 text-sm font-medium text-ink-300">
                  {item}
                </div>
              ))}
            </div>
            <div className="mt-8 rounded-md border border-ink-700 bg-ink-900 p-6">
              <div className="eyebrow text-signal-400">Architectural principle</div>
          ...
          <Reveal as="section" className="rounded-lg border border-ink-700 bg-ink-900 p-8">
            ...
            <h2 className="font-display text-3xl font-medium text-ink-50">{c.labels.authentication}</h2>
            <div className="mt-6 grid gap-5 lg:grid-cols-[0.95fr_1.05fr]">
              <div className="space-y-4">
                {[
                  ...
                  <div key={item} className="flex items-center gap-3 rounded-md border border-ink-700 bg-ink-850 px-4 py-4 text-sm text-ink-300">
                    <Check className="h-4 w-4 shrink-0 text-signal-400" />
                    <span>{item}</span>
                  </div>
                ))}
              </div>
...
Accept: application/json`}
            ...
            <div className="mt-8 grid gap-4 md:grid-cols-2">
              {[
                ...
                <div key={item} className="flex items-center gap-3 rounded-md border border-ink-700 bg-ink-850 px-4 py-4 text-sm text-ink-300">
                  <ShieldCheck className="h-4 w-4 shrink-0 text-signal-400" />
                  <span>{item}</span>
                </div>
              ))}
            </div>
          ...
          <Reveal as="section" className="rounded-lg border border-ink-700 bg-ink-900 p-8">
            ...
            <h2 className="font-display text-3xl font-medium text-ink-50">{c.labels.api}</h2>
            <div className="mt-6 overflow-x-auto rounded-lg border border-ink-700">
              <div className="grid min-w-[520px] grid-cols-3 bg-ink-850 text-sm font-semibold text-ink-50">
              ...
              {[
                ...
                <div key={row[0]} className="grid min-w-[520px] grid-cols-3 border-t border-ink-700 text-sm text-ink-300">
                  <div className="px-5 py-4 font-medium text-ink-50">{row[0]}</div>
            ...
            <div className="mt-8">
              <Link
                ...
                className="inline-flex h-12 items-center gap-2 rounded-md bg-signal-500 px-7 text-sm font-semibold text-ink-950 transition-all hover:bg-signal-400 hover:shadow-glow-sm"
              >
                {c.openApiRef}
            ...
            <div className="mt-8 overflow-x-auto rounded-lg border border-ink-700">
              <div className="grid min-w-[560px] grid-cols-4 bg-ink-850 text-sm font-semibold text-ink-50">
              ...
              {[
                ...
                <div key={row[0]} className="grid min-w-[560px] grid-cols-4 border-t border-ink-700 text-sm text-ink-300">
                  <div className="px-5 py-4 font-medium text-ink-50">{row[0]}</div>
                  <div className="px-5 py-4">{row[1]}</div>
                  <div className="px-5 py-4">{row[2]}</div>
          ...
          <Reveal as="section" className="rounded-lg border border-ink-700 bg-ink-900 p-8 shadow-panel">
            <span id="agents" className="-mt-24 block pt-24" aria-hidden />
            <h2 className="font-display text-3xl font-medium text-ink-50">{c.labels.agents}</h2>
            <p className="mt-4 max-w-3xl text-base leading-8 text-ink-300">
            ...
            <div className="mt-8 grid gap-4 sm:grid-cols-3">
              {["Discover", "Execute", "Submit"].map((item) => (
                <div key={item} className="rounded-md border border-signal-500/30 bg-signal-500/10 p-5 text-sm font-medium text-signal-400">
                  {item}
                </div>
              ))}
            </div>
            <div className="mt-8 space-y-3">
              {[
                ...
                <div key={item} className="rounded-md border border-ink-700 bg-ink-850 px-4 py-4 text-sm leading-7 text-ink-300">
                  {item}
                </div>
              ))}
            </div>
          ...
          <Reveal as="section" className="rounded-lg border border-ink-700 bg-ink-900 p-8">
            ...
            <h2 className="font-display text-3xl font-medium text-ink-50">{c.labels.webhooks}</h2>
            <p className="mt-4 max-w-3xl text-base leading-8 text-ink-300">
          ...
          <Reveal as="section" className="rounded-lg border border-ink-700 bg-ink-900 p-8">
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <h2 className="font-display text-3xl font-medium text-ink-50">{c.labels.examples}</h2>
                <p className="mt-3 text-base leading-8 text-ink-300">
                  Keep examples short, clear, and close to real workflows.
                </p>
              </div>
              <Link
                href="/resources/sdk"
                className="inline-flex h-12 items-center gap-2 rounded-md border border-ink-600 bg-transparent px-7 text-sm font-medium text-ink-100 transition-colors hover:border-ink-400 hover:bg-ink-800"
              >
                {c.sdkResources}
          ...
          <Reveal as="section" className="rounded-lg border border-ink-700 bg-ink-850 p-8">
            ...
            <h2 className="font-display text-3xl font-medium text-ink-50">{c.labels.operations}</h2>
            <div className="mt-6 grid gap-4 md:grid-cols-2">
              {[
                ...
                <div key={item} className="rounded-md border border-ink-700 bg-ink-900 px-5 py-5 text-sm leading-7 text-ink-300">
                  {item}
                </div>
              ))}
            </div>
