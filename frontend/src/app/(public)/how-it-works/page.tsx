    ...
    <div className="min-h-screen bg-ink-950 text-ink-50">
      ...
      <section className="px-4 pb-24 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-6xl">
          <Reveal className="eyebrow text-signal-500">{c.stepsEyebrow}</Reveal>
          <Reveal delay={70}>
            ...
            {c.steps.map((step, i) => {
              ...
              return (
                <Reveal
                  key={step.n}
                  delay={i * 70}
                  className="hover-lift flex flex-col rounded-lg border border-ink-700 bg-ink-900 p-7 hover:border-signal-500/40"
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-mono text-sm text-ink-500">{step.n}</span>
                  </div>
                  <h3 className="mt-5 text-lg font-semibold text-ink-50">{step.title}</h3>
                  <p className="mt-3 flex-1 text-sm leading-7 text-ink-400">{step.body}</p>
                  <div className="mt-5 flex items-start gap-2 border-t border-ink-800 pt-4">
                    <span className="eyebrow shrink-0 text-signal-400">{c.guaranteeLabel}</span>
                    <span className="text-sm leading-6 text-ink-200">{step.guarantee}</span>
                  </div>
                </Reveal>
              );
            })}
      ...
      <section className="relative border-y border-paper-ink/15 bg-paper px-4 py-20 text-paper-ink sm:px-6 lg:px-8">
        ...
        <div className="relative mx-auto max-w-6xl">
          <Reveal>
            <h2 className="font-display text-3xl font-medium text-paper-ink sm:text-4xl">
              {c.statsTitle}
            </h2>
          </Reveal>
          <Reveal delay={80}>
            <p className="mt-5 max-w-3xl text-lg leading-8 text-paper-ink/70">{c.statsBody}</p>
          </Reveal>
            ...
            {c.stats.map((stat, i) => {
              ...
              return (
                <Reveal
                  key={stat.label}
                  delay={i * 80}
                  className="hover-lift rounded-md border border-paper-ink/15 bg-paper p-7 hover:border-paper-ink/40"
                >
                  <Icon className="h-5 w-5 text-signal-600" />
                  <div className="mt-5 font-display text-5xl font-medium text-paper-ink">
      ...
      <section className="px-4 py-24 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-6xl">
          <Reveal className="eyebrow text-signal-500">{c.lifecycleEyebrow}</Reveal>
          <Reveal delay={70}>
            <h2 className="mt-3 font-display text-3xl font-medium text-ink-50 sm:text-4xl">
              {c.lifecycleTitle}
            </h2>
          ...
          <Reveal delay={140} className="mt-10 overflow-x-auto rounded-lg border border-ink-700 bg-ink-900 shadow-panel">
            <div className="min-w-[680px]">
              ...
              {c.lifecycleRows.map((row) => (
                <div
                  key={row[0]}
                  className="grid grid-cols-4 border-b border-ink-800 text-sm text-ink-400 last:border-b-0 hover:bg-ink-800/50"
                >
                  <div className="px-5 py-4 font-medium text-ink-50">{row[0]}</div>
                  <div className="px-5 py-4">{row[1]}</div>
                  <div className="px-5 py-4 font-mono text-xs text-signal-400">{row[2]}</div>
